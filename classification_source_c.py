...
def train():
    ...
    # 日志打印间隔为1/4
    log_interval = math.floor(train_data.shape[0]/bptt/4)
    # log_interval = 1
    # 开始遍历批次数据
...
# 只保存神经网络的模型参数
torch.save(best_model.state_dict(), './Net/Transformer_Testmachine_sourceclassfication_net_params.pkl')
...
for bptt_i in range(21,30,1):
    ...
    for epoch in range(1, epochs+1):
        ...
        # 记录验证准确率
        if epoch == epochs:
            bptt_eval_acc.append(val_accuracy)



    # 模型测试 依然使用evaluate函数 best_model以及测试数据
    test_loss, test_acc = evaluate(best_model, test_data, test_label)
    bptt_test_acc.append(test_acc)
